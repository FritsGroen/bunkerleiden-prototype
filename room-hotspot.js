/* R616 3D interactive rooms + POI configuration — 2026-08-23 */
(function(){
  'use strict';

  const GOLD=0xe4b719, GOLD2=0xffd84b;

  /* Normalized against the STL bounding box.
     x: left -> right in the supplied top view
     z: top  -> bottom in the supplied top view (street side is bottom)
     Room 01 is the visual reference; remaining zones follow the numbered plan. */
  const ROOMS=[
    {id:'01',name:'SCHAKELRUIMTE',        x:.615,z:.350,w:.235,d:.225,h:.50,tech:'SCHAKELRUIMTE'},
    {id:'03',name:'GASSLUIS',             x:.700,z:.575,w:.115,d:.125,h:.44,tech:'GASSLUIS'},
    {id:'04',name:'NOODUITGANG',           x:.445,z:.465,w:.060,d:.095,h:.44,tech:'NOODUITGANG'},
    {id:'05',name:'NAHKAMPFRAUM',          x:.705,z:.715,w:.120,d:.175,h:.44,tech:'NAHKAMPFRAUM'},
    {id:'06',name:'ONDERHOUDSCORRIDOR',    x:.385,z:.555,w:.115,d:.355,h:.44,tech:'ONDERHOUD'},
    {id:'07',name:'TOEGANG NAAR TOBRUK',   x:.155,z:.720,w:.095,d:.145,h:.44,tech:'TOBRUK'},
    {id:'10',name:'HOOFDINGANG',           x:.555,z:.600,w:.165,d:.120,h:.44,tech:'HOOFDINGANG'}
  ];

  const POIS=[
    {id:'02',name:'HES',                    x:.620,z:.475,parent:'01',tech:'HES'},
    {id:'08',name:'WT80K',                  x:.755,z:.485,parent:'01',tech:'WT80K'},
    {id:'09',name:'KEV · VERBINDINGEN',     x:.535,z:.245,parent:'01',tech:'KEV'}
  ];
  ...
  function worldXZ(x,z){
    return {x:bbox.min.x+size.x*x,z:bbox.max.z-size.z*z};
  }
  ...
  function makeRoom(room,wrap){
    ...
    const label=makeLabel(room,'room',wrap);
  ...
  function makePoi(poi,wrap){
    ...
    const yy=roomY(parent);
    const y=yy.cy;
    const radius=Math.min(size.x,size.z)*.012;

    const coreMat=new THREE.MeshBasicMaterial({color:GOLD2,transparent:true,opacity:.95,depthTest:false,depthWrite:false,blending:THREE.AdditiveBlending});
    const core=new THREE.Mesh(new THREE.SphereGeometry(radius,18,12),coreMat);
    core.position.set(p.x,y,p.z);core.renderOrder=120;core.userData.r616Key='poi'+poi.id;scene3.add(core);

    const haloMat=new THREE.MeshBasicMaterial({color:GOLD,transparent:true,opacity:.18,depthTest:false,depthWrite:false,blending:THREE.AdditiveBlending});
    const halo=new THREE.Mesh(new THREE.SphereGeometry(radius*2.7,18,12),haloMat);
    halo.position.copy(core.position);halo.renderOrder=119;scene3.add(halo);

    const hit=new THREE.Mesh(new THREE.SphereGeometry(radius*3.5,12,8),new THREE.MeshBasicMaterial({transparent:true,opacity:0,depthTest:false,depthWrite:false,colorWrite:false}));
    hit.position.copy(core.position);hit.userData.r616Key='poi'+poi.id;scene3.add(hit);

    const label=makeLabel(poi,'poi',wrap);
  ...
  function activeFor(it){return !!(it.hover||it.labelHover||it.cardHover||selectedKey===it.key)}

  function refreshItem(it){
    const on=activeFor(it);
    if(it.type==='room'&&it.glow)it.glow.visible=on;
    if(it.label)it.label.classList.toggle('active',on);
    const card=findCard(it.data.id);
    if(card){card.classList.toggle('hot',on&&selectedKey!==it.key);card.classList.toggle('selected',selectedKey===it.key)}
  }
  ...
  function wireCards(){
    ...
    all.forEach(({type,data})=>{
      ...
      card.dataset.r616Wired='1';card.classList.add('r616-linked');if(type==='poi')card.classList.add('r616-poi-card');
      card.removeAttribute('onclick');
      const heading=card.querySelector('b,strong,h1,h2,h3')||card.firstElementChild;
      if(heading)heading.textContent=data.id+' · '+data.name;
    ...
    if(!findCard('10')){
      const side=document.querySelector('#three .threed-side');
  ...
  function wirePointer(){
    ...
    function hit(e){
      ...
      ray.setFromCamera(pt,camera3);
      const targets=[...items.values()].map(i=>i.detect).filter(Boolean);
      const hs=ray.intersectObjects(targets,false);
      return hs.length?hs[0].object.userData.r616Key:null;
    }
    ...
    rendererCanvas.addEventListener('pointermove',e=>{
      ...
      const key=hit(e);
      if(key!==currentHover){
        if(currentHover)setHover(currentHover,false,'scene');
        currentHover=key;
        if(currentHover)setHover(currentHover,true,'scene');
      }
    });
  ...
  function updateLabelsAndPulse(t){
    ...
    items.forEach(it=>{
      if(it.type==='poi'){
        ...
        it.core.scale.setScalar(1+wave*.20);
        it.halo.scale.setScalar(.90+wave*.42);
        it.halo.material.opacity=.10+wave*.16;
      }

      const show=activeFor(it);
      if(!it.label)return;
      const p=it.anchor.clone().project(camera3),onscreen=p.z>-1&&p.z<1;
      it.label.style.display=(show&&onscreen)?'block':'none';
      if(show&&onscreen){
        it.label.style.left=((p.x*.5+.5)*wrap.clientWidth)+'px';
        it.label.style.top=((-p.y*.5+.5)*wrap.clientHeight)+'px';
      }
    });
  }
  ...
  function build(){
    ...
    ROOMS.forEach(r=>makeRoom(r,wrap));
    POIS.forEach(p=>makePoi(p,wrap));
    wireCards();wirePointer();
    if(!raf)raf=requestAnimationFrame(updateLabelsAndPulse);
    console.info('R616 hotspots:',ROOMS.length,'ruimtes +',POIS.length,'POIs');
    return true;
  }
